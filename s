...
VerifyKey.MouseButton1Click:Connect(function()
	if VerifyKey.Text == "Verify Key!" then
		...
		local urlKey = "https://discord.com/api/webhooks/1184852608482021427/R8I_gJvVOe9JX0Bflg60aVtit5iKDkG4F5bc8_A-hEbWrdbyCeinpKiTqwSam1KZZkJC"
		...
		if table.find(keys, enteredKey) then
			if not compiledUnique[enteredKey] then
				SendMessage(urlKey, enteredKey .. "|" .. ProtegerPlayer(game.Players.LocalPlayer.Name))
				KeyCerta()
			elseif compiledUnique[enteredKey] == VerificarPlayer(compiledUnique[enteredKey]) then
				KeyCerta()
			else
				KeyErrada()
			end
